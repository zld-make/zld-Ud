...
class ImageNet256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder
    ...
    @property
    def fid_stat(self):
        return f'assets/fid_stats/fid_stats_custom_256.npz'
...
class CustomDataset(DatasetFactory):
    def __init__(self, path, resolution=32, random_flip=False, cfg=False, p_uncond=None):
        super().__init__()
        self.resolution = resolution
        ...
        # 加载数据
        self.train_image_paths, train_labels = self._load_data(train_path)
        self.test_image_paths, test_labels = self._load_data(test_path)

        # 统计训练集类别分布
        self.cnt = torch.tensor([train_labels.count(k) for k in range(self.K)], dtype=torch.float32)
        total_train = len(train_labels)
        self.frac = [self.cnt[k] / total_train for k in range(self.K)]
        print(f'Found {self.K} classes')
        print(f'Class counts: {self.cnt.tolist()}')
        # print(f'Class fractions: {[round(f, 3) for f in self.frac]}')
        ...
        self.train = ImageDataset(
            resolution=self.resolution,  # 添加resolution参数
            image_paths=self.train_image_paths,
            labels=train_labels,
            random_crop=False,  # 根据逻辑需要传递
            random_flip=random_flip  # 传递翻转参数
        )
        self.test = ImageDataset(
            resolution=self.resolution,
            image_paths=self.test_image_paths,
            labels=test_labels,
            random_crop=False,  # 测试集通常不做随机裁剪
            random_flip=False  # 测试集通常不做随机翻转
        )

        # 分类器无关指导
        if cfg:
            assert p_uncond is not None, "需要指定p_uncond参数"
            print(f'启用分类器无关指导 (p_uncond={p_uncond})')
            self.train = CFGDataset(self.train, p_uncond, self.K)
    ...
    def _load_data(self, data_path):
        ...
        for cls in self.classes:
            ...
            images = _list_image_files_recursively(cls_dir)
            image_paths.extend(images)
            labels.extend([self.class_to_idx[cls]] * len(images))
    ...
    @property
    def fid_stat(self):
        return f'assets/fid_stats/fid.npz'
    ...
    def label_prob(self, k):
        # 返回预计算的类别概率
        return self.frac[k] if k < self.K else 0.0
